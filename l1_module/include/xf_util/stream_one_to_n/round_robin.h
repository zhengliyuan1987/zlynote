...
template <int _WInStrm, int _WOutStrm, int _NStrm>
void strm_one_to_n_rr(hls::stream<ap_uint<_WInStrm> >& istrm,
                   hls::stream<bool>& e_istrm,
                   hls::stream<ap_uint<_WOutStrm> > ostrms[_NStrm],
                   hls::stream<bool> e_ostrms[_NStrm]) {
/*
 * while a buffer is being filled by input data, another one is being read to ouput streams except the first time buff_a is not wrote yet. 
 * p is an iterator, range from 0 to  max(num_in, count_out), it executes at least one operation(buffer or output data) at each cycle.
 * when p=max, it means a buffer is consumed and another is full already,so switch them.  
 * e.g  _WInStrm=3, _WOutStrm=2, _NStrm=1  the width of buffer = 6 ,
 * ie 1 buffer = 2 input data = 3 ouput data,switch buffers every  max(3,2)=3 cycles.
 ...
 *    
 */
  // least common multiple of _WInStrm and _WOutStrm*_NStrm as the width of ping-pong buffer
    ...
    #pragma HLS pipeline II = 1
     ...
     if( p < num_in) {
      ...
      if(p< count_out && start) {
          ...
          #pragma HLS unroll
            ap_uint<_WOutStrm> d = sw ? buff_b.range((p*_NStrm+id+1)*_WOutStrm-1, (p*_NStrm+id)*_WOutStrm):
  ...
  int  id = 0;
  //  the output operation is suspended in the  while-loop when last=true, so  continully ouput data from the same buffer.
...
void strm_one_to_n(hls::stream<ap_uint<_WInStrm> >& istrm,
   ...
   details::strm_one_to_n_rr<_WInStrm, _WOutStrm, _NStrm>( istrm,
                       e_istrm,
                       ostrms,
                       e_ostrms);
}
