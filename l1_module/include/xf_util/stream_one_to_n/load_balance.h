 ...
 **************************************************************************************/
   ...
   while(!be) {
    ...
    // read state
     ...
     base         = next_base;
 /*
 * here ld = ld - rn2 means how many data are stored in buf_arr. However,if it locates before "if( ld < _NStrm)" , it leads big latency because the critical path includes  read the input stream. So buff_r2 is added to cache the input data. 
 *
 * */
   //ld = ld-rn2 ;
    int tc=0;
 ...
 * so it has to output the remaining data in buffers when the above while-loop quits.
 */

/* sequentially  output the remaining data in deq 
 * here, the next for-lopp will be unrolled to _NStrm while-loops, which will be run in sequence instead of in parallel. 
 */    
    int sid=0;
    for(int i=0; i< _NStrm;  ++i) {
      #pragma HLS unroll
...
strm_one_to_n_reduce<_WInStrm,_WOutStrm,_NStrm> (
                   buf_lcm_strm,
                   e_buf_lcm_strm,
                   left_lcm,
                   left_n,
...
strm_one_to_n_distribute<_WInStrm,_WOutStrm,_NStrm> (
                    buf_n_strm,
                    e_buf_n_strm,
                    left_n,
                    ostrms,
...
void strm_one_to_n(hls::stream<ap_uint<_WInStrm> >& istrm,
  ...
  details::strm_one_to_n_load_balance<_WInStrm, _WOutStrm, _NStrm>(
                     istrm,
                     e_istrm,
                     ostrms,
                     e_ostrms);
...
details::strm_one_to_n_load_balance_type<_TIn, _NStrm>(
                   istrm,
                   e_istrm,
                   ostrms,
                   e_ostrms);
