...
void dut(
  ...
  xf::util::level1::stream_combine(cfg, in, ein, out, eout, combine_right);
}
#else
...
int test_function(int len){
	...
	for (std::string::size_type i =0; i < len; i++){
		...
		for(int j = 0; j<_NStrm; j++){
			...
			if(testcfg[0][j] == 1){
				tmp((_NStrm - count) * _WIn - 1,(_NStrm - count -1) * _WIn) = testvector[i][j];
				count++;
			}
		}
		if(count < _NStrm)
		for(int j = count; j<_NStrm; j++){
			tmp((_NStrm - j) * _WIn - 1,(_NStrm - j -1) * _WIn) = (ap_uint<_WIn>)(0);
		}
