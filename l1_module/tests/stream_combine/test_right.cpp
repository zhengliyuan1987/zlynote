...
extern "C" void dut(
  ...
  xf::common::utils_hw::details::stream_combine(cfg, in, ein, out, eout, xf::common::utils_hw::combine_right_t());
}
  ...
  // TODO prepare cfg, in, ein; decl out, eout
	...
	for (std::string::size_type i =0; i < len; i++){
		...
		for(int j = 0; j<NSTRM; j++){
			...
			if(testcfg[0][j] == 1){
				tmp((NSTRM - count) * WIN - 1,(NSTRM - count - 1) * WIN) = testvector[i][j];
				count++;
			}
		}
		if(count < NSTRM)
		for(int j = count; j<NSTRM; j++){
			tmp((NSTRM - j) * WIN - 1,(NSTRM - j - 1) * WIN) = (ap_uint<WIN>)(0);
		}
