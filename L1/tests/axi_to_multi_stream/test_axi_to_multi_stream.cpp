...
void readbatchToPrintII1(hls::stream<_TStrm>& ostrm,
...
template <typename _TStrm>
void readbatchToPrintII2(hls::stream<_TStrm>& ostrm,
                         hls::stream<bool>& e_ostrm,
                         hls::stream<_TStrm>& r_strm,
                         const int num) {
    _TStrm dat;
...
template <typename _TStrm>
void readbatchToPrintII8(hls::stream<_TStrm>& ostrm,
                         hls::stream<bool>& e_ostrm,
                         hls::stream<_TStrm>& r_strm,
                         const int num) {
    _TStrm dat;
...
#pragma HLS INTERFACE m_axi port = rbuf depth = DDR_DEPTH offset = slave bundle = gmem_in1 latency = \
...
void top_for_co_sim(ap_uint<AXI_WIDTH>* rbuf,
                    const int len[3],
                    const int offset[3],
...
// function to print
template <typename _TStrm>
void PrintRowFile(_TStrm* rowDtmp_ap, int& num) {
...
template <typename _TStrm>
void PrintStrmRowFile(hls::stream<_TStrm>& r_strm, int& num) {
    std::ofstream fout("file.dat", std::ios::app);
    for (int i = 0; i < (num); i++) {
...
//    int offset[3]= {0,    0, 0};
    ...
    // err = load_dat<char>(dataInDDR, dataFile, in_dir,
    // (len_all+offset[0]+AXI_WIDTH/8-1)/(AXI_WIDTH/8)*(AXI_WIDTH/8));//16960
    ...
    top_for_co_sim((ap_uint<AXI_WIDTH>*)dataInDDR, len, offset, r_strm0, r_strm1, r_strm2, out_num[0], out_num[1],
                   out_num[2]);
