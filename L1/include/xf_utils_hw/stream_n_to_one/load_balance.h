...
/**
 * @file stream_n_to_one/load_balance.h
...
#pragma HLS unroll
      ...
      last[i] = (bak_last[i] || e_istrms[i].empty()) ? bak_last[i]
                                                     : e_istrms[i].read();
      ...
      std::cout << std::dec << "i= " << i << " last[i]= " << last[i]
                << std::endl;
#endif
    } // for
...
#pragma HLS unroll
      ...
      int ones = count_ones<up_nstrm>(
          v);       // it's similar to round robin  if ones always is i.
      int p = ones; // index of tmpb[i].range(), where  istrm[i] is stored if it
                    // is not empty
      ap_uint<_NStrm* _WInStrm> d = ttm[i];
      tmpb[i] = d << (p * _WInStrm);
#if !defined(__SYNTHESIS__) && XF_UTIL_STRM_1NRR_DEBUG == 1
      std::cout << std::hex << "v= " << v << " d= " << d
                << " tmpb[i]=" << tmpb[i] << std::endl;
      std::cout << std::dec << "i= " << i << " p= " << p
                << " p*_WInStrm=" << p * _WInStrm << std::endl;
#endif
    } // for
    buff_b = 0;
    for (int i = 0; i < _NStrm; ++i) {
#pragma HLS pipeline II = 1
    ...
    int un = count_ones<up_nstrm>(
        val); // how many new data are collected to buffer at this time

    // accumulate data
    if (un > 0) {
      buff_a.range((base + un) * _WInStrm - 1, base * _WInStrm) =
          buff_b.range(un * _WInStrm - 1, 0);
    }
    // output one data
    if (base + un >= num_in) {
...
#pragma HLS pipeline II = 1
  ...
  buf_lcm_strm.write(inner_buf);
  e_buf_lcm_strm.write(
      true); // even if true, the data in buf_strm is available.
 ...
 */
template <int _WInStrm, int _WOutStrm, int _NStrm>
void stream_n_to_one_load_balance(
    hls::stream<ap_uint<_WInStrm> > istrms[_NStrm],
    hls::stream<bool> e_istrms[_NStrm],
    hls::stream<ap_uint<_WOutStrm> >& ostrm,
    hls::stream<bool>& e_ostrm) {
...
#pragma HLS dataflow
  ...
  stream_n_to_one_read_lb<_WInStrm, _NStrm>(
      istrms, e_istrms, left_n, buf_n_strm, e_buf_n_strm);

  stream_n_to_one_collect_lb<_WInStrm, _WOutStrm, _NStrm>(
      buf_n_strm, e_buf_n_strm, left_n, left_lcm, buf_lcm_strm, e_buf_lcm_strm);

  stream_n_to_one_distribute_lb<_WInStrm, _WOutStrm, _NStrm>(
      buf_lcm_strm, e_buf_lcm_strm, left_lcm, ostrm, e_ostrm);
}
