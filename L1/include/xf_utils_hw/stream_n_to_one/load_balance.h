 ...
 * @param istrms input data streams.
 * @param e_istrms end flag streams for input data.
 * @param lef_n the number of available data in last buf_lcm_strm.
 ...
 */
 ...
 // output the last data(_NStrm*_WInStrm), which have base available data(_WInStrm bits)
 buf_lcm_strm.write( buff_a.range(buf_width-1,0));
 ...
 * @param left_lcm the number of available data in last buf_lcm_strm, output port.
 * @param buf_lcm_strm output stream.
 * @param e_buf_lcm_strm end flag stream.
 */
template <int _WInStrm, int _WOutStrm,int _NStrm>
                   ...
                   hls::stream<bool>& e_buf_lcm_strm)
{
  ...
  #if !defined(__SYNTHESIS__) && XF_UTIL_STRM_1NRR_DEBUG == 1
    std::cout<< "p="<<p<< "  "<< "left="<< left<<std::endl;
 ...
 * @param left_lcm the number of available data in last buf_lcm_strm, input port.
...
void stream_n_to_one_load_balance(hls::stream<ap_uint<_WInStrm> > istrms[_NStrm],
  ...
  hls::stream<ap_uint<32> > left_n; //how many input(_WInStrm bits) are stored in  last data(_NStrm*WInstrm bits) in buf_n_strm
  hls::stream<ap_uint<32> > left_lcm; 
 ...
 *  collect data: buffer  buf_width=lcm(_WInStrm*_NStrm, _WOutStrm) bits and output them.
 *  distribute  : output buf_width/_WOutStrm  data when read buf_width bits once
 *
