...
#pragma HLS pipeline II = 1
  ...
  e_buf_lcm_strm.write(
      true); // even if true, the data in buf_lcm_strm is available.
 ...
 */
template <int _WInStrm, int _WOutStrm, int _NStrm>
void stream_n_to_one_distribute(
    hls::stream<ap_uint<lcm<_WInStrm * _NStrm, _WOutStrm>::value> >&
        buf_lcm_strm,
    hls::stream<bool>& e_buf_lcm_strm,
    hls::stream<ap_uint<32> >& left_lcm,
...
#pragma HLS pipeline II = 1
    ...
    if (c == num_out) {
      ...
      if (last)
        up_pos =
            left_lcm.read(); // up_pos is changed only if the input will end
    }
...
#pragma HLS dataflow
  ...
  stream_n_to_one_read<_WInStrm, _NStrm>(
      istrms, e_istrms, left_n, buf_n_strm, e_buf_n_strm);

  stream_n_to_one_collect<_WInStrm, _WOutStrm, _NStrm>(
      buf_n_strm, e_buf_n_strm, left_n, left_lcm, buf_lcm_strm, e_buf_lcm_strm);

  stream_n_to_one_distribute<_WInStrm, _WOutStrm, _NStrm>(
...
void stream_n_to_one(hls::stream<ap_uint<_WInStrm> > istrms[_NStrm],
                     ...
                     hls::stream<bool>& e_ostrm,
                     round_robin_t alg) {
  details::stream_n_to_one_round_robin<_WInStrm, _WOutStrm, _NStrm>(
...
void stream_n_to_one(hls::stream<_TIn> istrms[_NStrm],
                     ...
                     hls::stream<bool>& e_ostrm,
                     round_robin_t alg) {
  details::stream_n_to_one_round_robin_type<_TIn, _NStrm>(
      istrms, e_istrms, ostrm, e_ostrm);
}
