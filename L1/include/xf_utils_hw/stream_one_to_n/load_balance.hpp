 ...
 */
template <int _WInStrm, int _WOutStrm, int _NStrm>
void stream_one_to_n_read(
    hls::stream<ap_uint<_WInStrm> >& istrm,
    hls::stream<bool>& e_istrm,
    hls::stream<ap_uint<lcm<_WInStrm, _NStrm * _WOutStrm>::value> >&
        buf_lcm_strm,
  ...
  std::cout << "lcm(" << _WInStrm << ", " << _WOutStrm << ")=" << buf_width
            << std::endl;
 ...
 */
template <int _WInStrm, int _WOutStrm, int _NStrm>
void stream_one_to_n_reduce(
    hls::stream<ap_uint<lcm<_WInStrm, _WOutStrm * _NStrm>::value> >&
        buf_lcm_strm,
    hls::stream<bool>& e_buf_lcm_strm,
    hls::stream<int>& left_lcm,
...
#pragma HLS pipeline II = 1
    ...
    for (int i = 0; i < count_out; ++i) {
      buf_n_strm.write(inner_buf.range((i + 1) * _NStrm * _WOutStrm - 1,
                                       i * _NStrm * _WOutStrm));
...
#pragma HLS pipeline II = 1
  ...
  buf_n_strm.write(inner_buf.range((flg + 1) * _NStrm * _WOutStrm - 1,
                                   flg * _NStrm * _WOutStrm));
 ...
 */
template <int _WInStrm, int _WOutStrm, int _NStrm>
void stream_one_to_n_distribute(
    hls::stream<ap_uint<_NStrm * _WOutStrm> >& buf_n_strm,
    hls::stream<bool>& e_buf_n_strm,
    hls::stream<int>& left_n,
    hls::stream<ap_uint<_WOutStrm> > ostrms[_NStrm],
...
#pragma HLS pipeline II = 1
      if (false == ostrms[sid].full()) {
        ostrms[sid].write(
            buff_r.range((ib + 1) * _WOutStrm - 1, ib * _WOutStrm));
        e_ostrms[sid].write(false);
        ib++;
      }
      sid = (sid + 1 == _NStrm) ? 0 : sid + 1;
...
#pragma HLS pipeline II = 1
    if (false == ostrms[sid].full()) {
      ostrms[sid].write(
          buff_r.range((ib2 + 1) * _WOutStrm - 1, ib2 * _WOutStrm));
      e_ostrms[sid].write(false);
      ib2++;
    }
    sid = (sid + 1 == _NStrm) ? 0 : sid + 1;
 ...
 */
template <int _WInStrm, int _WOutStrm, int _NStrm>
void stream_one_to_n_load_balance(
    hls::stream<ap_uint<_WInStrm> >& istrm,
    hls::stream<bool>& e_istrm,
    hls::stream<ap_uint<_WOutStrm> > ostrms[_NStrm],
    hls::stream<bool> e_ostrms[_NStrm]) {
...
#pragma HLS stream variable = e_buf_n_strm depth = 32
  ...
  *
  */
  stream_one_to_n_read<_WInStrm, _WOutStrm, _NStrm>(
      istrm, e_istrm, buf_lcm_strm, left_lcm, e_buf_lcm_strm);

  stream_one_to_n_reduce<_WInStrm, _WOutStrm, _NStrm>(
      buf_lcm_strm, e_buf_lcm_strm, left_lcm, left_n, buf_n_strm, e_buf_n_strm);

  stream_one_to_n_distribute<_WInStrm, _WOutStrm, _NStrm>(
      buf_n_strm, e_buf_n_strm, left_n, ostrms, e_ostrms);
}
...
void stream_one_to_n(hls::stream<ap_uint<_WInStrm> >& istrm,
                     ...
                     hls::stream<bool> e_ostrms[_NStrm],
                     load_balance_t alg) {
  details::stream_one_to_n_load_balance<_WInStrm, _WOutStrm, _NStrm>(
...
void stream_one_to_n(hls::stream<_TIn>& istrm,
                     ...
                     hls::stream<bool> e_ostrms[_NStrm],
                     load_balance_t alg) {
  details::stream_one_to_n_load_balance_type<_TIn, _NStrm>(
      istrm, e_istrm, ostrms, e_ostrms);
}
